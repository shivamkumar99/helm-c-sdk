...
func TestVerifyChartSigned(t *testing.T) {
	out, err := VerifyChart(
		filepath.Join(signingDir, "testchart-0.1.0.tgz"),
		"", // defaults to <path>.prov
		filepath.Join(signingDir, "pubring.gpg"),
	)
	require.NoError(t, err)
	...
	assert.Equal(t, "testchart-0.1.0.tgz", v.FileName)
...
func TestVerifyChartWithoutProvenance(t *testing.T) {
	...
	_, err := VerifyChart(tgz, "", filepath.Join(signingDir, "pubring.gpg"))
	require.Error(t, err)
