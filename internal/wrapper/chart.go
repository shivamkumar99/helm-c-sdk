// Package wrapper is the pure-Go use-case layer over the Helm SDK. All logic
// lives here, fully testable without cgo; the capi package only converts and
// delegates. Loggers, when needed, are injected — never global.
package wrapper
...
func CreateChart(name, dir string) (string, error) {
	if err := chartutil.ValidateMetadataName(name); err != nil {
