...
Prebuilt releases cover linux-amd64, darwin-arm64, and windows-amd64. Anywhere else — an
arm64 Linux box, an Intel Mac, Alpine/musl, FreeBSD — build it yourself; `make build` works
on every platform Go supports:

```bash
git clone https://github.com/shivamkumar99/helm-c-sdk && cd helm-c-sdk
make build            # -> build/libhelm_c.<ext> (+ include/helm_c.h)
```

Then point your consumer at the result. Language bindings take a path rather than searching
the loader path — for example the Python binding:

```bash
export HELM_C_LIB=/path/to/helm-c-sdk/build
```
